...
namespace ElBayt_ECommerce.WebAPI.Controllers
{
    ...
    public class ShopController : Controller
    {
        ...
        [HttpGet]
        [Route(nameof(GetProductData))]
        public async Task<ActionResult> GetProductData(string ProductName)
        {
            ...
            try
            {
                ...
                #region Logging info
                _logger.InfoInDetail(ProductName, correlationGuid, nameof(ShopController), nameof(GetProductData), 1, User.Identity.Name);
                #endregion Logging info

                var ProductData = await _elBaytServices.ShopService.GetProductData(ProductName);
                #region Result
                Response.Result = EnumResponseResult.Successed;
            ...
            catch (Exception ex)
            {
                ...
                _logger.ErrorInDetail($"newException {ProductName}", correlationGuid,
                    $"{nameof(ShopController)}_{nameof(GetProductData)}_{nameof(Exception)}", ex, 1, User.Identity.Name);
